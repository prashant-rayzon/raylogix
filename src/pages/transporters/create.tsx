...
export function CreateTransporterModal({ isOpen, onClose, onSuccess }: CreateTransporterModalProps) {
  ...
  const validateField = (name: string, value: string | File | null): boolean => {
    ...
    switch (name) {
      ...
      case 'password':
        ...
        else if (String(value).length < 8) newErrors.password = 'Password must be at least 8 characters';
        else delete newErrors.password;
        break;
      case 'confirmPassword':
        if (!value) newErrors.confirmPassword = 'Please confirm your password';
        else if (String(value) !== formData.password) newErrors.confirmPassword = 'Passwords do not match';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const requiredFields = ['name', 'email', 'phone', 'password', 'confirmPassword', 'companyName', 'gstNumber', 'address'];
    let hasError = false;
    ...
    try {
      ...
      await transportersService.create(submitData);
      
      toast({
        title: 'Transporter created',
        description: `Transporter ${formData.name} has been created successfully.`,
      });
      ...
      const message = err?.data?.message || err?.message || 'Failed to create transporter';
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-y-auto animate-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="sticky top-0 bg-white z-10 flex items-center justify-between p-6 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-primary">Create Transporter</h3>
              <p className="text-sm text-gray-500">Add a new transporter with login credentials</p>
            </div>
          </div>
          <button
            onClick={onClose}
...
export default CreateTransporterModal
